...
public class DefaultTyperDeduperTest {
  ...
  private List<DestinationInitialState<MockState>> initialStates;
  ...
  private final Migration<MockState> MIGRATION_REQUIRING_SOFT_RESET = new Migration<>() {
    ...
    public MigrationResult<MockState> migrateIfNecessary(DestinationHandler<MockState> destinationHandler,
                                                         ...
                                                         DestinationInitialState<MockState> state) {
  ...
  void setup() throws Exception {
    ...
    initialStates
        .forEach(initialState -> when(initialState.initialRawTableState()).thenReturn(new InitialRawTableState(true, true, Optional.empty())));
  ...
  void existingNonemptyTable() throws Exception {
    initialStates.forEach(initialState -> {
      ...
      when(initialState.isSchemaMismatch()).thenReturn(true);
      when(initialState.initialRawTableState()).thenReturn(new InitialRawTableState(true, true, Optional.of(Instant.parse("2023-01-01T12:34:56Z"))));
    });
  ...
  void existingNonemptyTableMatchingSchema() throws Exception {
    initialStates.forEach(initialState -> {
      ...
      when(initialState.isFinalTableEmpty()).thenReturn(false);
      when(initialState.isSchemaMismatch()).thenReturn(false);
      when(initialState.initialRawTableState()).thenReturn(new InitialRawTableState(true, true, Optional.of(Instant.now())));
    });
  ...
  void noUnprocessedRecords() throws Exception {
    initialStates
        .forEach(initialState -> when(initialState.initialRawTableState()).thenReturn(new InitialRawTableState(true, false, Optional.empty())));
  ...
  void unprocessedRecords() throws Exception {
    initialStates.forEach(initialState -> when(initialState.initialRawTableState())
        .thenReturn(new InitialRawTableState(true, true, Optional.of(Instant.parse("2023-01-23T12:34:56Z")))));
    ...
    when(destinationHandler.gatherInitialState(anyList()))
        .thenReturn(List.of(
            new DestinationInitialState<>(
                OVERWRITE_STREAM_CONFIG,
                true,
                new InitialRawTableState(true, true, Optional.of(Instant.ofEpochMilli(42))),
                true,
                false,
  ...
  /**
   * A test where we have multiple migrations, but none of them do anything. We shouldn't do anything
   * interesting here.
   */
  @Test
  void noopMigrations() throws Exception {
    typerDeduper = new DefaultTyperDeduper<>(
        sqlGenerator,
        destinationHandler,
        parsedCatalog,
        List.of(MIGRATION_REQUIRING_SOFT_RESET, MIGRATION_NOOP));
    ...
    // The overwrite stream still gets a new table
    verify(destinationHandler).execute(Sql.of("CREATE TABLE overwrite_ns.overwrite_stream_airbyte_tmp"));
